...
const USER_LOGIN_REFRESH = "USER_LOGIN_REFRESH";
...
export const userLoginMain = (user) => async (dispatch) => {
  ...
  try {
    //await sendRequest();
    let response = await PostRequest(Global.Login, user);
...
export const GetRefreshToken = () => async (dispatch) => {
  ...
  try {
    await sendRequest();
    let response = await GetRequest(Global.GetRefreshToken);

    if (response.status == 200) {
      dispatch({ type: USER_LOGIN_REFRESH, payload: response.data });
      wait(50).then(() =>
        dispatch({
          type: API_Request_Completed,
        })
...
export const Logout = () => async (dispatch) => {
  try {
    let response = await GetRequest(Global.Logout);
    if (response.status == 200) {
      dispatch({ type: CLEAR_LOGIN_STATE });
    }
  } catch (error) {
